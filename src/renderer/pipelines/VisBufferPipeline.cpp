...
void VisBufferPipeline::bind(VkCommandBuffer cb) {
    vkCmdBindPipeline(cb, VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipeline);
}

void VisBufferPipeline::pushConstants(VkCommandBuffer cb, const VisBufferPushConstants& pcs) {
    vkCmdPushConstants(cb, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(VisBufferPushConstants), &pcs);
}
...
void VisBufferPipeline::createPipeline() {
    ...
    auto fragShaderCode = VulkanContext::readFile(SHADERS_DIR "visbuffer_frag.spv");
    ...
    // 8. Configure Color Blending (no blending needed for integer outputs)
    ...
    // 11. Configure Dynamic Rendering target formats (R32G32_UINT)
    VkFormat visFormat = VK_FORMAT_R32G32_UINT;
    VkFormat depthFormat = context.getDepthFormat();
    VkPipelineRenderingCreateInfo renderingCreateInfo{};
    renderingCreateInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO;
    renderingCreateInfo.colorAttachmentCount = 1;
    renderingCreateInfo.pColorAttachmentFormats = &visFormat;
    ...
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create VisBuffer graphics pipeline!");
    }
    ...
    std::cout << "Successfully created Vulkan graphics pipeline inside VisBufferPipeline!" << std::endl;
}
